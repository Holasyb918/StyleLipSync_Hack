...
class MappingNetwork(torch.nn.Module):
    ...
    def forward(
        self, z, c=None, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False
    ):
...
class SaMF(torch.nn.Module):
    ...
    def forward(self, x_enc, x_dec, w, noise_mode="random", fused_modconv=True, gain=1):
        assert noise_mode in ["random", "const", "none"]
        in_resolution = self.resolution // self.up
        print(x_dec.shape, self.weight.shape)
        misc.assert_shape(x_dec, [None, self.weight.shape[1], in_resolution, in_resolution])
        styles = self.affine(w)

        noise = None
        flip_weight = self.up == 1  # slightly faster
        mask = modulated_conv2d(
            x=x_enc,
            weight=self.weight,
            styles=styles,
            noise=noise,
...
@persistence.persistent_class
class StyleLipSync_SynthesisBlock(torch.nn.Module):
    def __init__(
        self,
        in_channels,  # Number of input channels, 0 = first block.
        out_channels,  # Number of output channels.
    ...
    def forward(self, x, img, ws, enc_feat, force_fp32=False, fused_modconv=None, **layer_kwargs):
        misc.assert_shape(ws, [None, self.num_conv + self.num_torgb, self.w_dim])
        w_iter = iter(ws.unbind(dim=1))
        dtype = torch.float16 if self.use_fp16 and not force_fp32 else torch.float32
        memory_format = (
        ...
        if self.in_channels == 0:
            ...
            x = x.to(dtype=dtype, memory_format=memory_format)
            x = self.samf(enc_feat, x, next(w_iter), fused_modconv=fused_modconv, **layer_kwargs) # new
            w_iter = iter(ws.unbind(dim=1))
...
@persistence.persistent_class
class StyleLipSync_SynthesisNetwork(torch.nn.Module):
    def __init__(
        self,
        w_dim,  # Intermediate latent (W) dimensionality.
        img_resolution,  # Output image resolution.
        ...
        **block_kwargs,  # Arguments for SynthesisBlock.
        ...
        for res in self.block_resolutions:
            ...
            use_fp16 = res >= fp16_resolution
            is_last = res == self.img_resolution
            block = StyleLipSync_SynthesisBlock(
                in_channels,
                out_channels,
                w_dim=w_dim,
                resolution=res,
    ...
    def forward(self, ws, enc_feats, **block_kwargs):
        block_ws = []
        with torch.autograd.profiler.record_function("split_ws"):
            misc.assert_shape(ws, [None, self.num_ws, self.w_dim])
            ws = ws.to(torch.float32)
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            block = getattr(self, f"b{res}")
            enc_feat = enc_feats.get(res // 2, None)
            x, img = block(x, img, cur_ws, enc_feat, **block_kwargs)
        return img
...
@persistence.persistent_class
class StyleLipSync_G(torch.nn.Module):
    def __init__(
        self,
        w_dim,  # Intermediate latent (W) dimensionality.
        img_resolution,  # Output resolution.
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = StyleLipSync_SynthesisNetwork(
            w_dim=w_dim,
            img_resolution=img_resolution,
            img_channels=img_channels,
            **synthesis_kwargs,
    ...
    def forward(
        self, ws, enc_feats, truncation_psi=1, truncation_cutoff=None, **synthesis_kwargs
    ):
        img = self.synthesis(ws, enc_feats, **synthesis_kwargs)
        return img
